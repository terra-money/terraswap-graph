...
export class PairDataResolver {
  ...
  async pairData(@Arg('pairAddresses', (type) => [String]) pairAddresses: string[]): Promise<Partial<PairData>[]> {
    const pairData = await this.pairDataService.getPairData(pairAddresses)
